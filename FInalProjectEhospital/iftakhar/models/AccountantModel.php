...
function addAllEmployeeSalary($user)
{


    $conn = getConnection();
    $sql = "insert into allemployeeaddsalary values( '{$user['EmpASfirstName']}', '{$user['EmpASlastName']}', '{$user['EmpASuserName']}', '{$user['EmpASalary']}', '{$user['EmpASType']}')";
    $status = mysqli_query($conn, $sql);
    return $status;
}

function showAllEmployeeSalary()
{
    $conn = getConnection();
    $sql = "select * from allemployeeaddsalary ";
...
function showAllRegisteredPatient()
{
    $conn = getConnection();
...
function searchPersonForAddPayment($user)
{
    $conn = getConnection();
    //SELECT * FROM `phramacistaddproduct` WHERE productName='c1'
    $sql = "SELECT * FROM patientregistration where patientUserName='{$user['searchPerson']}'";
    $result = mysqli_query($conn, $sql);
...
function validForPay($user)
{
    $conn = getConnection();
    //SELECT * FROM `phramacistaddproduct` WHERE productName='c1'
    $sql = "SELECT * FROM patientpaymentdetails where patientUserName='{$user['searchPerson']}'";
    $result = mysqli_query($conn, $sql);
...
function patientPayCost($user)
{
    $conn = getConnection();
    $sql = "insert into patientpaymentdetails values( '{$user['firstName']}', '{$user['lastName']}', '{$user['userName']}', '{$user['pAdmissionDate']}', '{$user['pCost']}', '{$user['gender']}')";
    $status = mysqli_query($conn, $sql);
    return $status;
}

function uplaodFile($user)
{
    $conn = getConnection();
    $sql = "insert into importantfileupload values( '{$user['des']}')";
...
function showAllFile(){
    $conn = getConnection();
    $sql = "select * from importantfileupload ";
    $result = mysqli_query($conn, $sql);
    return $result;
}
